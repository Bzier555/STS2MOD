...
/// <summary>
/// Ported from <c>../STS2Ghostmod/STS2Ghostmod/src/AdaptedAi/Cards/CardCatalogBuilder.cs</c> (MIT,
/// sts2-ai-teammate) per <c>docs/AI.md</c> §2. Builds the static, process-lifetime semantic profile
/// of every card once, from <c>ModelDb.AllCards</c> — this is the "once," not "per turn" or "per card
/// play," expensive step <c>docs/AI.md</c>'s efficiency section calls out (POSTMORTEM F18); everything
/// downstream reads the result via <see cref="CardCatalogRepository.Shared"/>'s <c>Lazy</c>.
/// </summary>
internal sealed class CardCatalogBuilder
{
    public CardCatalogRepository Build()
    {
        ...
        {
            CardCatalogEntry entry = BuildEntry(card);
            repository.Upsert(entry);
            builtCount++;
        }

        GhostLog.Info($"AI card catalog built: {builtCount} entries.");
        return repository;
    }

    private static CardCatalogEntry BuildEntry(CardModel canonicalCard)
    {
