...
/// <summary>
/// Ported from <c>../STS2Ghostmod/STS2Ghostmod/src/AdaptedAi/Cards/CardResolver.cs</c> (MIT,
/// sts2-ai-teammate) per <c>docs/AI.md</c> §2 ("re-port from there, not from upstream"). Resolves a
/// live <c>CardModel</c> (upgrade level, run/combat overlays) against the static
/// <see cref="CardCatalogRepository"/> into a <see cref="ResolvedCardView"/> the scorer can read
/// without touching engine reflection again. No adversarial-specific logic lives here — a card's own
/// effects don't care which side is holding it.
/// </summary>
internal sealed class CardResolver : ICardResolver
{
    ...
    public ResolvedCardView Resolve(CardModel liveCard, string cardInstanceId)
    {
        ...
        {
            return ResolveFromCatalog(catalogEntry, cardInstanceId, upgradeLevel, isUpgraded, runOverlay, combatOverlay);
        }

        CardDefinition fallbackDefinition = GetOrCreateFallbackDefinition(liveCard);
        return ResolveFromFallback(fallbackDefinition, cardInstanceId, upgradeLevel, isUpgraded, runOverlay, combatOverlay);
    }
    ...
    private static IReadOnlyList<NormalizedEffectDescriptor> ExtractEffects(CardModel liveCard)
    {
        ...
        // Necrobinder's Bodyguard exposes its pet health through Summon rather
        // than Block. Treat that persistent health as defense for planning;
        // the native card still performs the exact summon at execution time.
        ...
        if (liveCard.Description.GetFormattedText().Contains("channel", StringComparison.OrdinalIgnoreCase))
        {
            effects.Add(new NormalizedEffectDescriptor
            {
                Kind = EffectKind.ChannelOrb,
                TargetScope = TargetScope.Self,
                Amount = 1,
                DurationHint = DurationHint.Immediate,
                ValueTiming = ValueTiming.Setup
